...
public class ShipCommand extends SlashCommand {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            // Fetch romantic background
            String bgUrl = klippyService.fetchGif("anime%20romance%20aesthetic%20love").orElse(null);
            if (bgUrl == null) {
                bgUrl = klippyService.fetchGif("romantic%20pink%20aesthetic").orElseThrow();
            }
            ...
            // Generate ship name
            String shipName = generateShipName(userA.getUser().getName(), userB.getUser().getName());
            ...
            EmbedBuilder embed = buildRomanticEmbed(userA, userB, percent, shipName, isSelfShip);
            ...
            event.getHook().sendMessage("💔 The love gods are having technical difficulties...").queue();
        }
    }

    private EmbedBuilder buildRomanticEmbed(Member userA, Member userB, int percent, String shipName, boolean isSelfShip) {
        ...
        embed.setDescription(
            "💕 **" + userA.getAsMention() + "** + **" + userB.getAsMention() + "** 💕\n\n" +
            "*\"" + loveQuote + "\"*\n\n" +
            "**Ship Name:** `" + shipName + "`\n" +
            "**Compatibility Score:** `" + percent + "%`\n" +
            tier.getEmoji() + " *" + tier.getDescription() + "*"
        );
    ...
    private BufferedImage generateRomanticImage(
            ...
            String shipName,
            String backgroundURL, int percent, boolean isSelfShip) throws Exception {
        ...
        // Soft romantic gradient overlay (pink/purple tones)
        GradientPaint gradient = new GradientPaint(
                0, 0, new Color(40, 20, 40, 200),
                width, height, new Color(60, 30, 60, 160)
        );
        g.setPaint(gradient);
    ...
    private ShipTier getShipTier(int percent) {
        if (percent >= 95) return new ShipTier("💍💍💍", "Destined Soulmates", "Written in the stars", new Color(255, 215, 0));
        if (percent >= 90) return new ShipTier("💍💍", "True Soulmates", "A love for the ages", new Color(255, 105, 180));
        if (percent >= 85) return new ShipTier("💍", "Perfect Match", "Marriage material!", new Color(255, 20, 147));
        if (percent >= 80) return new ShipTier("💕💕💕", "Power Couple", "The world envies you", new Color(220, 20, 60));
        ...
        if (percent >= 50) return new ShipTier("💝", "Potential Couple", "Something special brewing", new Color(147, 112, 219));
